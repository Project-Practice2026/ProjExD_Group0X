...
* **UI多言語日本語化対応・画面レイアウト最適化（担当：齋藤）** ：日本語化に伴うメインメニュー・IP入力画面の文字化けを `get_font` の共通化により解消。また、チュートリアルオーバーレイ画面におけるチェックボックス行とヒント文の位置計算ロジック定数化（`HINT_TOP_GAP`）による文字重なりバグの防止
